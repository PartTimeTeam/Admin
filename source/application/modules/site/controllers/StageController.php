<?php
class Site_StageController extends FrontBaseAction {
    public function init() {
        parent::init();
        $this->loadJs( array( 'pages-stage' ) );
    }
...
//     	}
    	...
    	$count = $mdlStage->fetchAllStage( $this->post_data );
    	//get filtered data
    	unset( $this->post_data['count_only'] );
    
    	$list = $mdlStage->fetchAllStage( $this->post_data );
    	if ( empty( $list ) == false ){
    ...
    public function detailAction(){
    	...
    	if( empty( $this->post_data ['id'] ) == false ) {
    		...
    		$info = $mdlStage->fetchStageById( $id );
    		if( empty( $info ) == true ) {
...
//     		}
    		$xml = APPLICATION_PATH.'/xml/stage.xml';
