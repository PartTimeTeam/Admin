    				...
    				if (isset ( $condition ['validate'] ) == true && is_array ( $condition ['validate'] )) {
    					foreach ( $condition ['validate'] as $className => $rule_item ) {
    						...
    						if ($className == 'NotEmpty') {
    							...
    							$constraint [$key] ['data-errormessage-value-missing'] = self::_getTranslate ( $rule_item ['message'], true );
    							...
    							if (isset ( $rule_item ['max'] ) == true) {
    								...
    								$constraint [$key] ['data-errormessage-range-overflow'] = self::_getTranslate ( $rule_item ['message'], true );
    							}
    							if (isset ( $rule_item ['min'] ) == true) {
    								$constraint [$key] ['data-validation-engine'] [] = "validate[minSize[" . ($rule_item ['min']) . "]]";
    								$constraint [$key] ['data-errormessage-range-underflow'] = self::_getTranslate ( $rule_item ['message'], true );
    							}
    							...
    							$constraint [$key] ['data-errormessage-password-mismatch'] = self::_getTranslate ( $rule_item ['message'], true );
    							...
    							$constraint [$key] ['data-errormessage-email-error'] = self::_getTranslate ( $rule_item ['message'], true );
    							...
    							$constraint [$key] ['data-errormessage-amount-error'] = self::_getTranslate ( $rule_item ['message'], true );
    							...
    							$constraint [$key] ['data-error-message'] = self::_getTranslate ( $rule_item ['message'], true );
